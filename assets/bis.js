  ...
  constructor(settings) {
    ...
    (0,defineProperty/* default */.Z)(this, "initSticky", () => {
      ...
      if (this.mainStickyAddToCart) {
        this.mainStickyAddToCart.addEventListener('change', () => {
          const selectedVariantId = this.mainStickyAddToCart.querySelector('[name="id"]').value;
          const selectedVariant = this.productData.variants.find(v => v.id === Number(selectedVariantId));
          this.settings.productData.current_variant_id = selectedVariant.id;
          this.toggleShowBISButtonSticky(selectedVariant);
        });
      }
    });
